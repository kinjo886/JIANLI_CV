...
public:
	...
	//渲染位置和距离上一次调用play函数的时间过了多久（delta）
...
public:
	...
	bool CheckPlayerCollision(const Player& player) {
		...
		bool is_over_x = check_position.x >= player_position.x && check_position.x <= player_position.x + player.FRAME_WIDTH;
		bool is_over_y = check_position.y >= player_position.y && check_position.y <= player_position.y + player.FRAME_HEIGHT;
		return is_over_x && is_over_y;
